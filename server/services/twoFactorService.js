...
class TwoFactorService {
  ...
  async sendOTP(phoneNumber) {
    try {
      ...
      // Using AUTOGEN2 ensures SMS-only delivery with approved DLT template
...
const twoFactorServiceInstance = new TwoFactorService();

// Export default instance
export default twoFactorServiceInstance;
...
export async function sendOTPviaSMS(phoneNumber, otp = null) {
  // If OTP is provided, we're using custom OTP (for forgot password)
  // If not, use 2Factor's auto-generated OTP
  if (otp) {
    // For custom OTP, we'll need to use a different approach
    // For now, use the auto-generated method
    return await twoFactorServiceInstance.sendOTP(phoneNumber);
  }
  return await twoFactorServiceInstance.sendOTP(phoneNumber);
}
